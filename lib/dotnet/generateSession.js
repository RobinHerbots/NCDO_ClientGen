...
module.exports = function (service, clientName, clientPath, catalogUrl) {
    if (service) {
        var content = [
            ...
            `   {`,
            `       public static  CDOSessionOptions DefaultOptions => new CDOSessionOptions() {ServiceUri = new Uri("${catalogUrl.href.replace(`/static/${service.name}.json`, "")}")};`,
            `       public ${clientName}(CDOSessionOptions options) : base(options)`,
            `       {`,
