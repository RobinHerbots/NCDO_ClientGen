...
function genABLTypeSetter(ablType, propertyName) {
    ...
    if (ablType) {
        switch (ablType.toUpperCase()) {
            case "DATE":
                getter.push(`           set { this["${propertyName}"] = value.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture); OnPropertyChanged("${propertyName}"); }`);
                break;
            case "DATETIME":
                getter.push(`           set { this["${propertyName}"] = value.ToString("yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture); OnPropertyChanged("${propertyName}"); }`);
                break;
            case "DATETIME-TZ":
                getter.push(`           set { this["${propertyName}"] = value.ToString("yyyy-MM-ddTHH:mm:sszzz", CultureInfo.InvariantCulture); OnPropertyChanged("${propertyName}"); }`);
                break;
            case "DECIMAL":
            case "INT64":
            case "INTEGER":
            case "LOGICAL":
            default:
                getter.push(`           set { this["${propertyName}"] = value; OnPropertyChanged("${propertyName}"); }`);
        }
    } else getter.push(`           set { this["${propertyName}"] = value; OnPropertyChanged("${propertyName}"); }`);
    return getter.join("\n");
}

function genDSFromSchema(dsName, dsSchema) {

    if (dsSchema.type === "object") {
        DDS.DS.push(dsName);
    }
...
function genTableFromSchema(tableName, tableSchema) {
    ...
    var content = [
        ...
        `{`,
        `    public class ${tableName} : CDO_Table<${recordName}>`,
        `    {`,
    ...
    console.log(`${_clientPath}${sep}Models${sep}Tables${sep}${tableName}.cs`);
    fs.writeFileSync(`${_clientPath}${sep}Models${sep}Tables${sep}${tableName}.cs`, content.join('\n'));
}
...
function genRecordFromSchema(recordName, recordSchema, primaryKeys) {
    ...
    console.log(`${_clientPath}${sep}Models${sep}Records${sep}${recordName}.cs`);
    fs.writeFileSync(`${_clientPath}${sep}Models${sep}Records${sep}${recordName}.cs`, content.join('\n'));
}
