...
function genABLTypeGetter(propObj, propertyName, backingField) {
	...
	if (ablType) {
		switch (ablType.toUpperCase()) {
			...
			case "DATETIME-TZ":
				if (propObj.type === "array")
					getter.push(`           get => this.Get("${propertyName}").ToArray().Select(d => (DateTime.TryParseExact(d, "yyyy-MM-ddTHH:mm:sszzz", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue);`);
				else
...
function genABLTypeSetter(propObj, propertyName) {
	...
	if (ablType) {
		switch (ablType.toUpperCase()) {
			...
			case "DATETIME-TZ":
				if (propObj.type === "array")
					getter.push(`           set => this["${propertyName}"].AddRange(value.ToString("yyyy-MM-ddTHH:mm:sszzz", CultureInfo.InvariantCulture));`);
				else
		...
		if (propObj.semanticType !== "Internal") {
			...
			if (propObj.default !== undefined) {
				if ((propObj.type === "string" || (propObj.type === "array" && propObj.items && propObj.items.type === "string")) && propObj.default !== null) {
					...
					constructorContent.push(`          this["${propName}"] = "${propObj.default}";`);
				} else {
					propertyContent.push(`       [DefaultValue(${propObj.default})]`);
