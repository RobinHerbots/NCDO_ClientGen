...
function genABLTypeGetter(ablType, propertyName, backingField) {
    var getter = [];
    if (ablType) {
        switch (ablType.toUpperCase()) {
            case "DATE":
                getter.push(`           get => ${backingField ? `_${propertyName};` : `DateTime.TryParseExact(this.Get("${propertyName}"), "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue;`}`);
                break;
            case "DATETIME":
                getter.push(`           get => ${backingField ? `_${propertyName};` : `DateTime.TryParseExact(this.Get("${propertyName}"), "yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue;`}`);
                break;
            case "DATETIME-TZ":
                getter.push(`           get => ${backingField ? `_${propertyName};` : `DateTime.TryParseExact(this.Get("${propertyName}"), "yyyy-MM-ddTHH:mm:sszzz", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue;`}`);
                break;
            case "DECIMAL":
            case "INT64":
            case "INTEGER":
            case "LOGICAL":
            default:
                getter.push(`           get => ${backingField ? `_${propertyName};` : `this.Get("${propertyName}");`}`);
        }
    } else getter.push(`           get => ${backingField ? `_${propertyName};` : `this.Get("${propertyName}");`}`);
    return getter.join("\n");
}

function genABLTypeSetter(ablType, propertyName, backingField) {
    var getter = [];
    if (ablType) {
        switch (ablType.toUpperCase()) {
            case "DATE":
                getter.push(`           set => this["${propertyName}"] ${backingField ? `= (_${propertyName}` : ""} = value).ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);`);
                break;
            case "DATETIME":
                getter.push(`           set => this["${propertyName}"] ${backingField ? `= (_${propertyName}` : ""} = value).ToString("yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture);`);
                break;
            case "DATETIME-TZ":
                getter.push(`           set => this["${propertyName}"] ${backingField ? `= (_${propertyName}` : ""} = value).ToString("yyyy-MM-ddTHH:mm:sszzz", CultureInfo.InvariantCulture);`);
                break;
            case "DECIMAL":
            case "INT64":
            case "INTEGER":
            case "LOGICAL":
            default:
                getter.push(`           set => this["${propertyName}"] ${backingField ? `= _${propertyName}` : ""} = value;`);
        }
    } else getter.push(`           set => this["${propertyName}"] ${backingField ? `= _${propertyName}` : ""} = value;`);
    return getter.join("\n");
}
...
function genRecordFromSchema(recordName, recordSchema, primaryKeys) {
    ...
    for (var rndx in recordSchema) {
        ...
        if (propObj.semanticType !== "Internal") {
            //set backend field
            content.push(`       private ${convertABLType(propObj.ablType)} _${rndx};`);
            if (propObj.title !== "") {
            ...
            var property = [
                ...
                "       {",
                `${genABLTypeGetter(propObj.ablType, rndx, true)}`,
                `${genABLTypeSetter(propObj.ablType, rndx, true)}`,
                "       }"
            ]
