...
function convertABLType(ablType, nullable) {
...
function genABLTypeGetter(ablType, propertyName, backingField) {
    ...
    if (ablType) {
        switch (ablType.toUpperCase()) {
            case "DATE":
                getter.push(`               return (_${propertyName} ?? (_${propertyName} = (DateTime.TryParseExact(this.Get("${propertyName}"), "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue))).Value;`);
                break;
            case "DATETIME":
                getter.push(`               return (_${propertyName} ?? (_${propertyName} = (DateTime.TryParseExact(this.Get("${propertyName}"), "yyyy-MM-ddTHH:mm:ss", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue))).Value;`);
                break;
            case "DATETIME-TZ":
                getter.push(`               return (_${propertyName} ?? (_${propertyName} = (DateTime.TryParseExact(this.Get("${propertyName}"), "yyyy-MM-ddTHH:mm:sszzz", CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : DateTime.MinValue))).Value;`);
                break;
            case "DECIMAL":
            case "INT64":
            case "INTEGER":
            case "LOGICAL":
                getter.push(`               return (_${propertyName} ?? (_${propertyName} = this["${propertyName}"])).Value;`);
                break;
            default:
                getter.push(`               return _${propertyName} ?? (_${propertyName} = this["${propertyName}"]);`);
        }
    } else getter.push(`               return _${propertyName} ?? (_${propertyName} = this["${propertyName}"]);`);
    getter.push("           }");
    return getter.join("\n");
}
...
function genABLTypeSetter(ablType, propertyName) {
    ...
    } else getter.push(`           set => this["${propertyName}"] = _${propertyName} = value;`);
    return getter.join("\n");
}
