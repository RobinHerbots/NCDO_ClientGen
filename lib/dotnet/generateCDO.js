...
module.exports = function (resource, clientName, clientPath) {
	...
	function genOperation(operation) {
		function genParams(operation) {
			...
			if (paramStr !== "") paramStr += ", ";
			paramStr += `CancellationToken cancellationToken = default(CancellationToken)`;
		...
		function genParams2(operation) {
			...
			for (var paramNdx in operation.params) {
				...
				if (param.type === "REQUEST_BODY") {
					if (DDS.DS.indexOf(param.name) !== -1)
						params.push(`            paramObj.Add("${param.name}", ${param.name});`);
					else if (DDS.TT.indexOf(param.name) !== -1)
						params.push(`            paramObj.Add("${param.name}", ${param.name});`);
					else {
						params.push(`            if (string.IsNullOrEmpty(${param.name}))`);
						params.push(`               paramObj.Add("${param.name}", null);`);
						params.push(`            else paramObj.Add("${param.name}", ${param.name});`);
					}
				}
			}
			...
			for (var paramNdx in operation.params) {
				...
				if (param.type === "RESPONSE_BODY") {
					if (DDS.DS.indexOf(param.name) !== -1)
						...
						responseParams.push(`cdoRequest.Response.Get("${param.name}").AsString()`);
				}
			}
			if (responseParams.length > 1)
		...
		content = content.concat([
			`           ICDORequest cdoRequest = await Invoke("${operation.name}", paramObj, cancellationToken);`,
