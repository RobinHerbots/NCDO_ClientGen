...
module.exports = function (resource, clientName, clientPath) {
    ...
    function genOperation(operation) {
            ...
            for (var paramNdx in operation.params) {
                ...
                if (param.type === "RESPONSE_BODY") {
                    if (param.name.charAt(0) === "d") {
                        return `new ${param.name}(cdoRequest.Response.Get("${param.name}") as IEnumerable<KeyValuePair<string, JsonValue>>)`;
                    } else {
                        return `cdoRequest.Response.Get("${param.name}")`;
                    }
                }
            }
            return "";
        ...
        content = content.concat([
            ...
            `           return ${genResponseParam2(operation)};`,
            "       }",
        ]);
        return content;
    }
    ...
    function genCDOBase(resource) {
        ...
        content = content.concat([
            ...
            "using UserManServiceClient.Models;",
            "using UserManServiceClient.Models.Tables;",
            "",
            `namespace ${clientName}.CDO`,
            "{",
            `    public class ${resource.name} : ACloudDataObject<JsonObject, ${dsName}>`,
            "   {",
