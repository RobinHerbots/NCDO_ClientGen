...
module.exports = function (grunt) {
    grunt.registerMultiTask('ncdo_clientgen', 'ncdo_clientgen task', function () {
        ...
        opts.output = grunt.option('output') || opts.output || process.cwd()
        ...
        opts.output = path.relative(process.cwd(), opts.output);

        //check options
        if (opts.catalogUrl === undefined) {
            grunt.log.writeln("required catalogUrl option");
            grunt.log.writeln("optional:");
            grunt.log.writeln("       output : output path for the client");
            ...
            cg(opts.catalogUrl, opts.output, opts.noclean, opts.framework, opts.bearer).then(function (data) {
